...
- Аудит и feedback сериализованы: append-only ревизии, durable write перед
  публикацией памяти/метрик, ошибки I/O возвращаются API; отмена запроса не
  прерывает начатый commit.
- Постоянный аудит требует `ASMODEUS_AUDIT_SIGNING_KEY`; ключ отделён от
  временного ключа каталога. Повреждённый/неподтверждённый журнал блокирует startup.
- Run ID используют случайный namespace процесса; метрики восстанавливаются
  из актуальных ревизий, отчёт читает один snapshot, суммы не переполняют `u64`.
- Safety-валидация внешних манифестов выполняется до их регистрации/подписи.
- Недоступные/draining раннеры исключены из маршрутизации; gRPC имеет deadlines,
  terminal event завершает ожидание без EOF; lifecycle-события получают общий ID.
- Верификация подписи отклоняет Unicode hex без panic.
- Добавлен обзор `docs/architecture-review-2026-09-22.ru.md` с незакрытыми
  ограничениями safety, trust boundary, отчётности и production-эксплуатации.
