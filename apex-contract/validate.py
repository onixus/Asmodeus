...
EXPECTED_SYSTEM = "asmodeus"
EXPECTED_NAMESPACE = "asmodeus"
...
def main() -> None:
    ...
    if doc.get("identity", {}).get("owning_service_authorizes_mutations") is not True:
        fail("Asmodeus must authorize its own mutations")
    if doc.get("ownership", {}).get("gateway_is_source_of_truth") is not False:
        fail("APEX Gateway must not own Asmodeus run state")
    safety = doc.get("safety", {})
    if safety.get("synthetic_only") is not True or safety.get("gateway_may_not_bypass_safety") is not True:
        fail("synthetic-only safety boundary drift")
