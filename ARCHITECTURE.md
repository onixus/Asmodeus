...
| D5 | **RBAC-инвариант обеспечивает control-plane, а не шлюз** | CISO / Auditor / SecOps получают `403` на любой запуск/инъекцию даже с валидным JWT. Control-plane проверяет APEX v1 Bearer JWT (`iss` / `aud` / подпись / TTL / contract version) самостоятельно; `X-Apex-Role` выключен по умолчанию и разрешается только явным demo opt-in. |
